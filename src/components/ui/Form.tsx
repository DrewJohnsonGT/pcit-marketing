...
const FormLabel: React.FC<React.ComponentProps<typeof LabelPrimitive.Root>> = ({ className, ...props }) => {
  ...
  return (
    <div className="flex h-6 items-center justify-between">
      <Label
        className={cn('text-base leading-[0.9]', error && 'text-destructive', className)}
        htmlFor={formItemId}
        {...props}
      />
    </div>
...
const FormRequiredIndicator: React.FC<React.ComponentProps<'span'>> = ({ className, ...props }) => {
  return (
    <span className={cn('text-xl font-bold text-orange-500', className)} {...props}>
      *
    </span>
  );
};
...
> = ({ children, className, hidePlaceholder = false, ...props }) => {
  const { error, formMessageId } = useFormField();
  const body = error ? String(error?.message) : children;

  if (!body && !hidePlaceholder) {
