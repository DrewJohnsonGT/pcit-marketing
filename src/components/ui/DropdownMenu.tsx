...
const DropdownMenuSubContent: React.FC<React.ComponentProps<typeof DropdownMenuPrimitive.SubContent>> = ({
  ...
  <DropdownMenuPrimitive.SubContent
    className={cn(
      `
        z-50 min-w-32 overflow-hidden rounded-md border bg-input-background p-1 text-popover-foreground shadow-lg
...
const DropdownMenuContent: React.FC<React.ComponentProps<typeof DropdownMenuPrimitive.Content>> = ({
  ...
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={cn(
        `
          z-50 min-w-32 overflow-hidden rounded-md border bg-input-background p-1 text-popover-foreground shadow-md
          data-[side=bottom]:slide-in-from-top-2
          data-[side=left]:slide-in-from-right-2
          data-[side=right]:slide-in-from-left-2
          data-[side=top]:slide-in-from-bottom-2
...
> = ({ className, inset, ...props }) => (
  <DropdownMenuPrimitive.Item
    className={cn(
      `
        relative flex h-input cursor-pointer items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden
...
const DropdownMenuCheckboxItem: React.FC<React.ComponentProps<typeof DropdownMenuPrimitive.CheckboxItem>> = ({
  ...
  <DropdownMenuPrimitive.CheckboxItem
    className={cn(
      `
        relative flex cursor-pointer items-center rounded-sm py-1.5 pr-2 pl-8 text-sm outline-hidden transition-colors
        select-none
        focus:bg-accent focus:text-accent-foreground
        data-disabled:pointer-events-none data-disabled:opacity-50
      `,
